...
#if	defined(ODM_MSTC) || defined(CONFIG_ODM_MSTC)
#define PROC_ENTRY_EXT_NAME1 "gponPhy_ext_eeprom0"
#define PROC_ENTRY_EXT_NAME2 "gponPhy_ext_eeprom1"
#define PROC_ENTRY_EXT_NAME3 "gponPhy_ext_eeprom2"
#define PROC_ENTRY_EXT_NAME4 "gponPhy_ext_eeprom3"
#endif	/* ODM_MSTC || CONFIG_ODM_MSTC */
...
#define GPON_PHY_I2C_ADDR3 0x50
#define GPON_PHY_I2C_ADDR4 0x51
#define GPON_PHY_I2C_ADDR5 0x52
#define GPON_PHY_I2C_ADDR6 0x53

/* Addresses to scan */
static unsigned short normal_i2c[] = { GPON_PHY_I2C_ADDR1, GPON_PHY_I2C_ADDR2, GPON_PHY_I2C_ADDR3, GPON_PHY_I2C_ADDR4,
	GPON_PHY_I2C_ADDR5, GPON_PHY_I2C_ADDR6, I2C_CLIENT_END };
#else
...
static struct gponPhy_data *pclient5_data;
static struct gponPhy_data *pclient6_data;
#endif	/* ODM_MSTC || CONFIG_ODM_MSTC */
...
static int get_client(u8 client_num, struct i2c_client **client)
{
	...
	case 3:
		*client = &pclient4_data->client;
		break;
	case 4:
		*client = &pclient5_data->client;
		break;
	case 5:
		*client = &pclient6_data->client;
		break;
	default:
...
#define LOG_WR_KBUF_SIZE 128
    ...
    case 'c':    
		...
		case GPON_PHY_I2C_ADDR4:
			client_num = 3;
			break;
		case GPON_PHY_I2C_ADDR5:
			client_num = 4;
			break;
		case GPON_PHY_I2C_ADDR6:
			client_num = 5;
			break;
		}
		break;
...
static int gponPhy_detect(struct i2c_adapter *adapter, int address, int kind)
{
	...
	case GPON_PHY_I2C_ADDR4:
		...
		break;
	case GPON_PHY_I2C_ADDR5:
		pclient5_data = pclient_data;
		break;
	case GPON_PHY_I2C_ADDR6:
		pclient6_data = pclient_data;
		break;
	default:
		goto exit_detach;
    }
    ...
    case GPON_PHY_I2C_ADDR2:
        ...
        break;
	case GPON_PHY_I2C_ADDR3:
		p = create_proc_entry(PROC_ENTRY_EXT_NAME1, 0, q);
		break;
	case GPON_PHY_I2C_ADDR4:
		p = create_proc_entry(PROC_ENTRY_EXT_NAME2, 0, q);
		break;
	case GPON_PHY_I2C_ADDR5:
		p = create_proc_entry(PROC_ENTRY_EXT_NAME3, 0, q);
		break;
	case GPON_PHY_I2C_ADDR6:
		p = create_proc_entry(PROC_ENTRY_EXT_NAME4, 0, q);
		break;
	default:
		goto exit_detach;
    }
	...
	/* __MSTC__: not accpet write internal eeprom */
	if ((address == GPON_PHY_I2C_ADDR1) || (address == GPON_PHY_I2C_ADDR2))
...
#if	defined(ODM_MSTC) || defined(CONFIG_ODM_MSTC)
        remove_proc_entry(PROC_ENTRY_EXT_NAME1, q);
        remove_proc_entry(PROC_ENTRY_EXT_NAME2, q);
        remove_proc_entry(PROC_ENTRY_EXT_NAME3, q);
        remove_proc_entry(PROC_ENTRY_EXT_NAME4, q);
#endif	/* ODM_MSTC || CONFIG_ODM_MSTC */
#ifdef GPON_I2C_TEST
...
static int gponPhy_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    p->read_proc = gponPhy_proc_read;
	/* __MSTC__: not accpet write internal eeprom */
	if ((client->addr == GPON_PHY_I2C_ADDR1) || (client->addr == GPON_PHY_I2C_ADDR2))
		p->write_proc = gponPhy_proc_write;
    p->data = (void *)pclient_data;
#else
    p->read_proc = gponPhy_proc_read;
